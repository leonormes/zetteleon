...
## 3. The Thinking Workflow (Refinement)

To transform "Noise" (Anxiety) into "Signal" (Knowledge/Action), we use the **Refinement Loop**:

1.  **Capture (The Dump):** Get the simulation out of the brain. Write it down in a `HEAD` note. (See [[SoT - PRODOS (System Architecture)#A. HEAD Notes (The Workbench)|HEAD Note Definition]]).
2.  **Decouple (The Pause):** Identify the *Trigger* (Spark) and the *Model* (Hypothesis).
3.  **Simulate (The Work):** Manipulate the variables in the note, not the head.
4.  **Resolve (The Output):**
    -   **If Actionable:** Create a **Next Test** (Action).
    -   **If Insight:** Synthesize into an **SoT** (Knowledge).

## 4. Synthesis: From Volatile to Stable
