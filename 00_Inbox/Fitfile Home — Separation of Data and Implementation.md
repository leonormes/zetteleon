...
type: note
tags: [fitfile, code-review, architecture, data-vs-implementation, linus-torvalds]
...
`packages/Brewfile` and `packages/apt-packages.txt` — the package manifest as data. The OS install scripts each read their corresponding file. The OS-branching `if` in bootstrap is acceptable conditional logic; the *what* to install is pure data.
...
The schema for runtime secrets. Every env var the system needs, documented with *where to get it*. This is the closest thing to a type system the bash world gets.
