*Shared Message from Pieces (https://pieces.app) by Leon Ormes (leonormes@gmail.com) on Tuesday May 26, 2026 - 2:49 PM*
---
I now have sufficient evidence from both tools to synthesise a comprehensive answer. The `search_memory` call with the targeted keywords (`prodOS Thinking Stream`, `prodOS Autonomous Action System`) returned the richest, most directly relevant results — including verbatim content from your `SoT - ProdOS Thinking Stream` note and the `Protocol - Autonomous Action System`. Combined with the earlier pages, I have all I need to deliver the full answer.

---

## How to use the prodOS workflow
...
This is the "RAM" of the system — a low-friction loop for converting raw mental noise into action or structured knowledge. The governing rule is the **Rule of Zero Toil**: if it feels like "organising", stop. The loop is for *thinking and doing only*.
