...
integration suite against it. Both jobs succeeding *is* a genuine "this version deployed and
...
**For real customer/production nodes:** the *shape* of the answer (a `PostSync` ArgoCD hook Job)
...
*real* customer-node promotion. `release.sh`/`release-improved.sh` in this repo handle tagging and
...
The one place `can-i-deploy` *can* run today without new infrastructure is the advisory job
