...
ifnotapi_key: 
    raiseValueError("OPENAI_API_KEY must be configured.")
...
```python
Python
defclassify_request(question: str) -> str:
    q =question.lower()
...
defcache_key(question: str, sources: list[str]) -> str:
    """
    ...
    returnhashlib.sha256(fingerprint.encode()).hexdigest()
...
defbuild_context(chunks):
    ...
    forchunk inchunks:
        ...
        ifused +tokens > MAX_CONTEXT_TOKENS:
            break
...
```python
Python
fromlangchain_core.documents importDocument
...
defvalidate_response(answer: str, sources: list[str]):
...
fromlangchain_core.prompts importChatPromptTemplate
 
logger =logging.getLogger(__name__)
 
defrun_pipeline(question: str):
    ...
    try:
        ...
        source_ids =[
            ...
            fordoc indocs
            ifdoc.metadata.get("source")
        ]
 
        # Check exact-match cache.
        key =cache_key(question, source_ids)
 
        ifkey inexact_match_cache:
            returnexact_match_cache[key]
