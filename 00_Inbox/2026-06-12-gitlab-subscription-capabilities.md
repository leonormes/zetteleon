---
tags: [gitlab, subscription, cicd, plan, premium, runner, quota, optimization]
...
| **Merge Pipelines** | `merge_pipelines_enabled: true` | Runs pipeline on the *merged result* (not just the source branch) — catches merge conflicts before they reach `development`. |
...
|---|---|---|---|
| **Forward Deployment** | `ci_forward_deployment_enabled: false` | Prevents a newer pipeline from being overwritten by an older one on the same ref. When pushing multiple commits rapidly, the *latest* pipeline always reflects the newest code — no stale results. | Prevents "last push won race" scenarios. If 2 pushes happen in quick succession, without this the older pipeline's result can overwrite the newer one. **Low effort, moderate reliability gain.** |
...
| **SaaS Runner Concurrency** | ~30 jobs | ~50+ jobs | Current bottleneck is *per-job duration*, not concurrency |
...
*(Find the deployment project ID with `glab api /projects/fitfile%2Fdeployment 2>&1 | jq '.id'`)*
