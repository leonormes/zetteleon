...
* **Cloud Agnostic (for backup storage):** While it uses cloud provider CSI drivers for snapshots, the backup *target* (where the backup metadata and filesystem backups are stored) can be an object store on any cloud (e.g., AWS S3, Azure Blob Storage, MinIO on-premises).9 This gives you flexibility in where you store your backups.  
