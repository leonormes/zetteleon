*Shared Message from Pieces (https://pieces.app) by Leon Ormes (leonormes@gmail.com) on Saturday Apr 11, 2026 - 9:35 AM*
---
# Additions to the Data-First Customer IaC Plan

This is a solid plan — the diagnosis is accurate and the target principles are sound. But cross-referencing against your month of audit work (the [MKUH Brutal Synthesis](https://obsidian.md), the [MASTER_REMEDIATION_PLAN.md](file:///Volumes/DAL/Fitfile/gitlab/FITFILE/New_Customer/MASTER_REMEDIATION_PLAN.md), the [CUSTOMER_DEPLOYMENT_MODULE_INTERFACE.md](file:///Volumes/DAL/Fitfile/gitlab/FITFILE/New_Customer/CUSTOMER_DEPLOYMENT_MODULE_INTERFACE.md), and the full Claude Code pipeline audit), there are meaningful gaps and sequencing issues. Here's what I'd add:

---

## 1. Pre-Requisite: Fix Live Bugs *Before* Module Extraction
...
- **Option A:** Accept that `infra_facts` is a "normalized config blob" and treat it as the single contract surface between Terraform and CUE. The shared module's job is to produce a *complete, well-typed* `infra_facts` from the merge of `customer.yaml` + `common.yaml` + provisioned state.
...
> *Update `cue/mock_infra.json` so it contains the complete `platform_vault` object and representative `vault_secret_consumers`. The mock must be a faithful representation of the contract.*
...
> *Optional: validate `customer.yaml` in CI with the same CUE/JSON Schema used by the module*
