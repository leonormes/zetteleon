...
modified: 2026-01-18T08:46:58+00:00
---
...
Because you have ADHD, I will keep this structured, scannable, and focused on the practical "How" and "Why" rather than getting bogged down in abstract theory.

Here is a review of how specific properties of a modern filesystem (FS) can be utilised to optimise LLM context efficiency for codebases.
...
- **The FS-DB Way:** Use **Vertical Slice Architecture**8. Group files by **Feature**, not technical type.
...
- **Broken Links:** If you move a file, the "Primary Key" changes. You must use tools (linters) to ensure imports update automatically22222222.
...
Here is how we break the code. Note that **Imports become Foreign Keys**2222.
...
You asked how this compares to "Industry SOTA". The industry is currently obsessed with **GraphRAG** and **LSP (Language Server Protocol)**.
...
**Required Adjustments for Viability:**
...
4. **Context Synthesis (Final Output)**
...
* SELECT \* FROM orders WHERE user\_id \= '123' involves a join or a secondary index lookup. In FS-DB, this corresponds to following a symlink or parsing a frontmatter reference.
...
When a GraphRAG system indexes this file, it parses the relates\_to field and creates an edge in the knowledge graph. This enables **Multi-Hop Retrieval**: when the user asks about "Checkout," the system automatically retrieves "Inventory" context because of this explicit link.
...
| **Standard MVC** | /app/controllers/api/v1/users\_controller.rb | 14 | Medium (Structural noise) |
...
* **Ordering:** We can use numeric prefixes to enforce a "Narrative Order" for the LLM.  
  * 00\_types.ts (Read First: Definitions)  
  * 01\_logic.ts (Read Second: Implementation)  
  * 02\_view.tsx (Read Third: Usage) This provides a "Chain of Thought" directly in the file listing, guiding the LLM's attention mechanism through the logical flow of the feature.
