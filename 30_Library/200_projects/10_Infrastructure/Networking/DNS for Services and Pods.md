...
- Note: This is not supported on Windows. See [below](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/?__readwiseLocation=#dns-windows) for details
- "`None`": It allows a Pod to ignore DNS settings from the Kubernetes environment. All DNS settings are supposed to be provided using the `dnsConfig` field in the Pod Spec. See [Pod's DNS config](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/?__readwiseLocation=#pod-dns-config) subsection below.
