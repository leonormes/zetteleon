...
modified: 2026-04-22T16:29:01+00:00
status: "refining"
...
- Canonical SoT: [[SoT - The Data-Centric Theory of Networking]]
- Related: [[SoT - Protocol Data Units (PDU)]], [[SoT - Linux Networking Primitives]]
...
Networking is a hierarchy of data labelling systems:
...
The "network" is the organisational schema, not the medium.

See [[SoT - Protocol Data Units (PDU)]] for the canonical breakdown of how each layer's headers (labels) are structured and nested.
...
- Creating ICMP data
- Labelling it with destination IP 8.8.8.8
...
- Are the labelling rules correct? (routing tables, ARP tables, iptables)
...
Kubernetes networking is entirely about labelling and relabelling:
...
NAT is not "network address translation" as in "translating between networks." It's relabelling:
...
See: [[SoT - Linux Networking Primitives]] §IPTables/NAT
...
See: [[SoT - Linux Networking Primitives]] §Routing
...
"Networking is hierarchical data labelling. Physical transmission is just the final step."
...
The OSI / TCP-IP layer model is a labelling taxonomy. Each layer defines:

- What labels to add (headers)
- What labels to read (for forwarding decisions)
- What labels to strip (decapsulation)

See: [[SoT - Protocol Data Units (PDU)]] for the canonical PDU structure at each layer.
...
See: [[MOC - Container Networking Model]], [[SoT - Linux Networking Primitives]]
...
- How does VXLAN add outer labels for overlay networking? (see [[SoT - Linux Networking Primitives]])
- How do service meshes intercept and relabel Layer 7 traffic?
- What is the performance cost of metadata inspection and relabelling?
- How do zero-trust networks use labels (identities) instead of IPs? (see [[SoT - Zero Trust Architecture]])
