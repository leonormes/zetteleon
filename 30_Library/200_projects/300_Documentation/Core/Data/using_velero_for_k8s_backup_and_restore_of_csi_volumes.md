...
We’ve covered off prepping and installing K8s on this blog a few different ways; with
...
If you just want to see it running - check out my
...
The Velero CLI is pre-compiled and available for download on the
...
To install Minio we’re going to use

Let’s create the `yaml` file for the setup of Minio with helm (a full list of variables can be found on the chart page
...
Stepping through this, it will deploy the latest version of Minio available, set the username and password to `minio` and `minio123` respectively, expose the service using a `LoadBalancer` (consequently, you’ll need a `LoadBalancer` of some kind in your cluster - I recommend

Ideally, instead of using Service Type `LoadBalancer` - you’d use an `Ingress Controller` like
...
![Minio](https://blah.cloud/automation/using-velero-for-k8s-backup-and-restore-of-csi-volumes/images/minio.png)
...
If you want to deploy via the CLI, see the
...
The last line may be something you’re wondering about - `deployRestic` tells Velero to deploy the
...
To test this out, let’s deploy an app - a Slack clone i’m awfully fond of called
...
![RocketChat](https://blah.cloud/automation/using-velero-for-k8s-backup-and-restore-of-csi-volumes/images/rocketchat.png)
...
![RocketChat Contents](https://blah.cloud/automation/using-velero-for-k8s-backup-and-restore-of-csi-volumes/images/data.png)
...
![Data in Minio](https://blah.cloud/automation/using-velero-for-k8s-backup-and-restore-of-csi-volumes/images/minio-data.png)
...
![CNS Volumes](https://blah.cloud/automation/using-velero-for-k8s-backup-and-restore-of-csi-volumes/images/cns-ui.png)
...
![CNS UI Empty](https://blah.cloud/automation/using-velero-for-k8s-backup-and-restore-of-csi-volumes/images/cns-ui-empty.png)
...
![CNS UI Post Restore](https://blah.cloud/automation/using-velero-for-k8s-backup-and-restore-of-csi-volumes/images/cns-ui-post-restore.png)

And our app should once be again accessible and our data safe:

![RocketChat Post Restore](https://blah.cloud/automation/using-velero-for-k8s-backup-and-restore-of-csi-volumes/images/rocketchat-post-restore.png)
...
This brings us to the end of our look at Velero on vSphere - and in particular the integration with CSI. If you have feedback for the Velero team - please reach out on
