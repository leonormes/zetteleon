...
2. Configure policies on your Vault server to grant or forbid access to certain
...
3. Configure [roles on your Vault server](#configure-server-roles), restricting roles to a project or namespace.
4. Create the following [CI/CD variables](../variables/_index.md#for-a-project)
...
- Use the `VAULT_CACERT` environment variable to configure GitLab Runner to trust the certificate:
  ...
  - If you deployed GitLab Runner using the [Helm chart](https://docs.gitlab.com/runner/install/kubernetes/):
    ...
    2. Add the following lines in your `values.yaml` file:
