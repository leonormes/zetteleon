...
1. In GitHub, create a token:
   ...
   3. Enter a Token description and update the scope to allow
...
3. In GitHub, add a `.gitlab-ci.yml` to [configure GitLab CI/CD](../quick_start/_index.md).

GitLab:

1. Imports the project.
2. Enables [pull mirroring](../../user/project/repository/mirror/pull.md).
3. Enables [GitHub project integration](../../user/project/integrations/github.md).
...
2. In GitLab, create a project:
   ...
   3. In the Git repository URL field, enter the HTTPS URL for your GitHub repository.
      If your project is private, use the personal access token you just created for authentication.
   4. Fill in all the other fields and select Create project.
      GitLab automatically configures polling-based pull mirroring.
3. In GitLab, enable [GitHub project integration](../../user/project/integrations/github.md):
...
4. In GitLab, create a personal access token with `API` scope to
   authenticate the GitHub web hook notifying GitLab of new commits.
5. In GitHub, from Settings > Webhooks, create a web hook to notify GitLab of
   new commits.
