...
tags: [cmd, argocd, exec, helm, oci, auth]
---

# Test Helm Registry Login from Repo-Server Pod

## 🎯 Intent
Test registry authentication explicitly from *inside* the `argocd-repo-server` pod. This isolates network boundary issues (e.g., Egress restrictions from the ArgoCD namespace to the ACR endpoint) while proving the helm binary inside ArgoCD can negotiate the TLS handshake successfully.
