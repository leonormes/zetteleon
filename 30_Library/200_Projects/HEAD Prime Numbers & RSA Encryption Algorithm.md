...
This is the part that trips many people up, but the video handles it elegantly. To figure out how to _reverse_ the scrambling process, the person generating the keys has to build a completely separate, secret "maths clock". This is where Euler's Totient Function comes in.

For a product of two primes, Euler's totient is calculated by taking the number _just before_ each prime and multiplying them together:
...
When you want to send a message, you use the public numbers (5 and 91) to encrypt it. To decrypt it, you absolutely _must_ know the secret companion number (29).
