...
distinguishes_from: ["[[SoT - Structural Intelligence|RepoMap]]", "[[SoT - Context Rot|Context Accumulation]]"]
modified: 2026-07-27T15:46:00+00:00
...
1. **Enforcement of Invariants**: While tools like a [[SoT - Structural Intelligence|RepoMap]] provide the structural skeleton of *what exists*, The Architectural Guardian dictates *what is permitted*. It encodes rules such as layers of isolation, data immutability, and framework strictures (the [[SoT - Dimensions of Code Understanding|Constraint Dimension]]).
...
- **Macro Boundaries**: Explicit demarcations between architectural layers (e.g. *"Service interfaces must never import UI models"*).
- **Negative Constraints**: Proscriptions against common LLM fallacies or local regressions (e.g. *"Never bypass the central authentication router"*).
