...
- **It's a different axis of division from the vault's existing Prompt-vs-Flow split**: [[SoT - Flow Engineering]] divides LLM control into two tiers (prompt-based vs. code-enforced orchestration) along a *mechanism* axis (talking the model into compliance vs. structurally forcing it). This note's four-stage hierarchy divides along a *task-duration* axis instead — the two framings are complementary, not competing, and could in principle be cross-referenced (Flow Engineering's orchestrator model is arguably what powers this note's Harness and Loop stages).
