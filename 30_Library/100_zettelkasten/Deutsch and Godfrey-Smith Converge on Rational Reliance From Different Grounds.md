---
aliases: []
conformant: true
contradicts: []
created: 2026-09-08T20:15:00+00:00
created_utc: 2026-09-08 20:15:00+00:00
epistemic_status: medium
evidence_links: []
modified: 2026-09-14T11:52:00+00:00
permalink: llmeon/00-inbox/deutsch-and-godfrey-smith-converge-on-rational-reliance-from-different-grounds
proposition: Facing the same "why is it rational to rely on a well-tested theory" dilemma that motivates Godfrey-Smith's confirmation objection, Deutsch argues we are justified in relying on a well-tested theory not because past observations confirmed it, but because it is the only rationally tenable explanation left once bad or unexplained alternatives are ruled out.
source_title: David Deutsch and Peter Godfrey-Smith
source_url: UNKNOWN
status: seed
tags: [deutsch, epistemology, godfrey-smith, philosophy-of-science, tension]
title: Deutsch and Godfrey-Smith Converge on Rational Reliance From Different Grounds
type: claim
---

## Deutsch and Godfrey-Smith Converge on Rational Reliance From Different Grounds

Interestingly, Deutsch explicitly addresses a dilemma identical to Godfrey-Smith's bridge example, using a scenario where a person considers jumping off a tower. Deutsch argues that we are justified in relying on the prevailing theory of gravity not because past observations "confirmed" it, but because it is the only rationally tenable theory remaining that provides a good explanation, whereas alternative theories provide bad or unexplained assertions.

### Scope & Conditions

Directly ties Deutsch's tower-jumping example to Godfrey-Smith's bridge-building example as two answers to the same underlying question: why is it rational to act on a well-tested theory?

### Evidence

> "Deutsch argues that we are justified in relying on the prevailing theory of gravity not because past observations 'confirmed' it, but because it is the only rationally tenable theory remaining that provides a good explanation, whereas alternative theories provide bad or unexplained assertions"

### Implications

- The two thinkers reach practically compatible conclusions (rational reliance on tested theories) via incompatible justifications: best-remaining-explanation (Deutsch) versus procedural confirmation (Godfrey-Smith).
- This is a genuine, unadjudicated tension, not a contradiction: both survive once you drop the shared assumption that rational reliance requires positive confirmation.

### Related

- [[A Good Explanation Is Hard to Vary]]—Deutsch's side of this tension rests entirely on this criterion: a theory is trustworthy because rival explanations are bad or unexplained, not because it was confirmed.

### Tensions

[refines:: [[Godfrey-Smith's Confirmation Objection to Strict Falsificationism]], confidence=medium]

- [[Godfrey-Smith's Confirmation Objection to Strict Falsificationism]]—that note argues rational reliance requires confirmation through testing; this note shows Deutsch reaching the same practical conclusion (it's rational to rely on the tested theory) while explicitly denying that confirmation is what grounds it. Assumption difference: whether rational reliance requires positive confirmation, or can rest purely on being the best surviving explanation.
- [[Predictive Power Verifies a Theory via Its Outputs, Not Its Proofs]]—a third, independent route to the same practical conclusion (trust the tested theory), grounded in output reliability rather than either confirmation or explanatory depth—suggesting the vault currently holds three distinct, unreconciled justifications for the same everyday trust judgement.
