...
title: Grammar-Constrained Decoding Forces Hallucination When JSON Tool-Call Sampling
  Fails
type: claim
...
proposition: When a model's structured-output decoding is grammar-constrained to produce
  valid JSON for a tool call, a single incorrect token sample (e.g. sampling a comma
  where none should follow) can force the decoder to hallucinate an entirely new key
  to keep the output grammatically valid JSON, poisoning the context window and breaking
  subsequent edits.
...
- [[Error Handling and Retry Pipelines for LLM Failures]]—related: detecting this failure mode requires validating *semantic* plausibility of tool-call fields, not just schema conformance, before accepting output.
