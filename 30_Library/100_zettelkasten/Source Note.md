---
conformant: true
created: 2026-09-26T10:30:00+00:00
created_utc: '2026-09-26T10:30:00Z'
definition: A Source Note summarises and paraphrases the passage that caught your
  attention, beginning the move from the words of the source to your own.
distinguishes_from: ["[[Point Note]]"]
modified: 2026-09-26T08:45:41+00:00
permalink: llmeon/30-library/100-zettelkasten/source-note
source_title: How to Make Notes and Write - Dan Allosso
source_url: calibre://view-book/GCcalibreBooks/708/EPUB
status: seed
tags: [allosso, definition, note-types, paraphrase, source-note, zettelkasten]
title: Source Note
type: concept
upstream: '[[The Atomicity Principle - One Idea Per Note]]'
used_in_claims: ["[[A Note Should Be Written So It Can Be Understood Out of Context]]"]
---

## Source Note

A Source Note summarises and paraphrases the passage that caught your attention, beginning the move from the source's words to your own. The source is Dan Allosso, How to Make Notes and Write.

### Scope & Conditions

Allosso's term for the second stage. He quotes only rarely, and says some notes are partly data and partly interpretation.

### Evidence

> "In the second stage, which I call a Source Note, you'll summarize and paraphrase the passage that grabbed your attention. Occasionally you might quote a line or two, if they seem to especially capture the thought. But the point is to begin the transition to your own words, which is the transition to your own thoughts."
> (Allosso, How to Make Notes and Write, chapter on writing as thinking)

### Implications

- A Source Note is closer to data and a Point Note closer to analysis.
- Quotation is the exception in a Source Note.

### Related

- [[Point Note]]—distinction: the later stage, which is about your own thoughts
- [[Literature Notes Bridge Source Material and Main Notes]]—shared mechanism: the same role in another vocabulary, a note about the source
- [[Fleeting Notes Are Temporary Capture Mechanisms]]—extends: the highlights that come before it are capture-stage notes
- [[A Note Should Be Written So It Can Be Understood Out of Context]]—extends: the out-of-context rule applies to it

### Tensions

- [[SoT - Atomic Note Standard (The Proposition Card)]]—tension, not contradiction: a Source Note paraphrases and only rarely quotes, while the Evidence section of a card quotes the source verbatim. Both can hold because a Source Note is a source-shaped note and the card is the idea note built from it.
