---
aliases: [The Stimulus-Response Gap]
conformant: true
created: 2025-09-07T15:32:45+00:00
definition: A response is a chosen action selected in the space between stimulus and behaviour; a reaction is an automatic discharge that closes that space.
last_reviewed: ''
modified: 2026-08-13T10:54:52+00:00
permalink: llmeon/30-library/100-zettelkasten/responding-vs.-reacting
status: ''
tags: [agency, emotional-regulation, mushin]
title: Responding vs. Reacting
type: concept
updated: null
---

_Mushin No Shin_ enables individuals to respond thoughtfully rather than react impulsively. For example, grabbing an umbrella when it rains is a response, while getting angry about the rain is a reaction. The goal is to cultivate the space between stimulus and response.

The distinction only makes sense if that space exists at all—the gap is what a response is selected _in_. That is the premise this note contributes to the argument graph.

Links: [[Self-Regulation is Managing Your Emotions and Actions]], [[Understanding Emotion]], [[Mushin No Shin Mind Without Mind]], [[Musashi's Duel with Sasaki Kojiro]]

## Knowledge Graph

Used in claims:

- [[Humans Retain Freedom to Choose Their Response to Any Situation Regardless of Circumstances]]—Frankl arrives at the same gap from a different tradition and asserts that it survives any circumstance. That claim `depends_on` this distinction; the edge is emitted there.

This note is a premise leaf: it supplies a distinction, not an argued claim, so it emits no justification edge of its own.

---

[[Miyamoto Musashi]]
