---
aliases: []
conformant: true
created: 2025-10-20T09:02:00+00:00
definition: "The habit of personal leadership — starting each day, task, or project with a clear vision of the desired destination, on the principle that everything is created twice: first mentally, then physically."
distinguishes_from: []
modified: 2026-07-28T09:12:46+00:00
permalink: llmeon/30-library/100-zettelkasten/habit-2-begin-with-the-end-in-mind
tags: [7-habits, gtd, leadership, principles, productivity, TheHuman/Habits, vision]
title: Habit 2 - Begin with the End in Mind
type: concept
used_in_claims: []
---

Habit 2: Begin with the End in Mind is the principle of personal leadership. It means starting each day, task, or project with a clear vision of your desired destination. It is the habit of creating a personal mission statement or philosophy.

This habit is based on the principle that all things are created twice:

1. The Mental Creation: The first creation is intellectual, in the mind. You define what you want to accomplish.
2. The Physical Creation: The second creation is the physical production of the thing you envisioned.

[[Habit 1 - Be Proactive]] says "You are the creator." %%[extends:: [[Habit 1 - Be Proactive]], confidence=high]%% Habit 2 is the first creation. It's about ensuring your actions are guided by a clear, personal compass.

## ProdOS Integration

This habit is the core of the upper levels of the [[LLM GTD Context|05 - Horizons of Focus]] model, specifically Horizon 5 (Purpose & Principles) and Horizon 4 (Vision). %%[depends_on:: [[LLM GTD Context]], confidence=medium]%% It is about defining your [[SoT - Goal Architecture (Vision and Alignment)|12 - Core Principles and Values]] before you begin to act. %%[depends_on:: [[SoT - Goal Architecture (Vision and Alignment)]], confidence=medium]%%

In the [[MOC - ProdOS]], this is formalized through commands like `/plan-life-vision`, which are designed to help articulate this personal constitution. The [[Protocol - Action-First GTD (LLM Chief of Staff)|07 - LLM as a Productivity Partner (CoS)]] facilitates this process of mental creation.

Once the vision is set, the next step is to manage your time and actions to make that vision a reality, which is the focus of [[Habit 3 - Put First Things First]]. %%[supports:: [[Habit 3 - Put First Things First]], confidence=high]%%
