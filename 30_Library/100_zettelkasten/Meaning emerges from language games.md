---
aliases: [forms of life, meaning in use, Wittgensteinian pragmatics]
conformant: true
created: 2025-10-31T20:05:00+00:00
epistemic_status: high
modified: 2026-07-28T09:12:48+00:00
permalink: llmeon/30-library/100-zettelkasten/meaning-emerges-from-language-games
prodos.kind: atomic
prodos.lifecycle: stable
proposition: Meaning is not fixed definitions or mental-to-mental transmission, but emerges from how language is used in practice within shared social activities (language games).
tags: [philosophy-of-language, pragmatics, Wittgenstein, communication]
title: Meaning emerges from language games
type: claim
---

## Meaning Emerges from Language Games

Summary: According to Wittgenstein, meaning emerges from how language is used in practice within social activities ("language games") rather than from perfect correspondence between words and thoughts.

Details: Wittgenstein proposed that the meaning of words is not fixed definitions but their use within particular "language games" - shared social practices where words gain practical meaning. We learn language through participation in these games, not through explicit definition. Communication succeeds when it's "good enough" for coordination within a specific context or activity. This explains why perfect understanding isn't necessary - we only need sufficient functional overlap to accomplish shared goals. The concept challenges the notion that successful communication requires perfect mental-to-mental transmission.

%%[supports:: [[SoT - Communication & Misunderstanding (The Experiential Filter)]]]%%

## Related

- [[MOC - The Gap Between Thought and Language]]
