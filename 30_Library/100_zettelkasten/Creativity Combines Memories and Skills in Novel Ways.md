---
aliases: []
confidence: 0.8
created: 2025-10-31T09:52:00Z
epistemic: fact
last_reviewed: 
modified: 2025-10-31T10:42:03Z
purpose: "Define creativity as a component of intelligence."
review_interval: 90
see_also: ["The Neuroscience of Developer Cognition.md"]
source_of_truth: []
status: seedling
tags: [cognition, creativity, problem-solving]
title: Creativity Combines Memories and Skills in Novel Ways
type: concept
uid: 
updated: 
---

## Creativity Combines Memories and Skills in Novel Ways

**Summary:** Creativity is a key component of intelligence that involves combining existing knowledge and skills in new ways to solve problems.

**Details:** Creative problem-solving:

- Draws on stored memories and learned skills
- Recombines these elements in innovative ways
- Can produce unexpected solutions to novel challenges

Examples include:

- Bees learning to move a ball for a reward
- Raccoons solving multi-step puzzles
- Humans using tools in unconventional ways

Creativity exists on a spectrum across species, with humans having particularly advanced capabilities in this domain.
