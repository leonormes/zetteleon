---
aliases: []
conformant: true
created: 2026-09-04T09:24:58+00:00
created_utc: '2026-09-04T14:00:00+00:00'
distinguishes_from: ["[[Falsifiability Distinguishes Science from Dogma]]"]
modified: 2026-09-06T17:53:00+00:00
permalink: llmeon/00-inbox/trusting-a-scientific-authority-feels-psychologically-identical-to-trusting-a-false-one
source_title: New chat
source_url: https://gemini.google.com/app/295a599fc8b28e35
status: seed
tags: [epistemology, philosophy-of-science, testimony, trust]
title: Trusting a Scientific Authority Feels Psychologically Identical to Trusting a False One
  a False One
type: concept
upstream: '[[HEAD New chat]]'
used_in_claims: []
---

## Trusting a Scientific Authority Feels Psychologically Identical to Trusting a False One

This is the "epistemology of testimony" problem: since no one can personally verify more than a fraction of human knowledge, belief is delegated to authorities. The subjective feeling of trusting a physicist because their explanation "makes sense" is functionally identical, from the inside, to a flat-earther trusting a video because it "makes sense" to them.

### Scope & Conditions

Applies to any belief held on testimony rather than personal verification—which, for any one person, is nearly all specialist knowledge.

### Evidence

> "You are absolutely right that, on a purely psychological level, your decision to trust a physicist feels functionally identical to a flat-earther's decision to trust a YouTube video. Both of you are delegating your belief to an authority because it 'makes sense' to you."

### Implications

- The feeling of certainty cannot itself be used to distinguish justified from unjustified belief.
- Justification has to be located in properties of the trusted system (see [[Falsifiability Distinguishes Science from Dogma]]), not in how confident the feeling is.

### Related

- [[The Realization that Knowledge is Contingent Can Lead to a Crisis of Certainty]]—shared mechanism: both describe the destabilising realisation that belief rests on delegated trust rather than personal verification.

[depends_on:: [[Falsifiability Distinguishes Science from Dogma]], confidence=high]

- [[Falsifiability Distinguishes Science from Dogma]]—this note's own resolution to the problem it poses: since the felt confidence of trust cannot distinguish justified belief from unjustified belief, the justification must live in a structural property of the system being trusted—falsifiability is that property.
