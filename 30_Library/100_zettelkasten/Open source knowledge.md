...
> What this note is. The hub of a four-note cluster contrasting two 17th-century approaches to establishing knowledge. Before this refresh it was a bare list of four wikilinks with no framing, and the cluster was a closed component of the graph—every link pointed inward, nothing connected it to the rest of the vault. Which is, given the subject, an unusually literal demonstration of the thesis.
...
- 2026-07-26—The note was four bare wikilinks with no prose. Rebuilt as a four-stage argument with typed edges; each stage now says what it contributes rather than merely appearing in a list.
- Closed component opened. All four children linked only to each other and back here. Added four outward relations connecting the cluster to the systems, epistemology and social-dynamics clusters. The cluster's own thesis predicts this was a problem.
- Frontmatter: `type: 'null' → map`; removed dead `status: 'null'`, `last_reviewed: 'null'`, `updated: null`; the sole tag was `topic/maths`, which described one of the four children—replaced with the cluster's actual subject. `conformant: false` flags the folder mismatch only.
- Duplicate spotted 2026-07-26, actioned 2026-08-01: [[Strong Opinions, Loosely Held Balances Conviction and Humility]] and `Strong Opinions Loosely Held Balances Confidence With Humility` were two notes making the same claim with near-identical bodies. Merged into the former (higher backlink count); the latter is now a tombstone redirect.
