...
The claim is a scope boundary, not a mechanism: it doesn't explain in detail _why_ context engineering breaks down past short-duration tasks (that's closer to the general context-degradation mechanics already covered elsewhere in this vault), it simply asserts that the boundary exists and names it as the reason a further layer (harness engineering) becomes necessary.
