...
**The PDU terminology you provided is 100% accurate.** Every layer designation—from Application layer "data" down to Physical layer "bits"—aligns precisely with authoritative networking sources GeeksforGeeks including Cisco NetAcad, RFC specifications, IEEE standards, and CompTIA certification materials. Protocol Data Units form the foundation of network encapsulation, where each layer wraps the data from above with its own control information, creating a nested structure that enables global internetworking across heterogeneous network technologies. Wikipedia HowToNetwork
...
The Data Link layer receives IP packets and must deliver them to the next physical hop on the path to the destination. **On Ethernet, switches use MAC addresses to forward frames only to the appropriate port**. The switch learns MAC addresses by observing source MAC addresses in received frames and building a MAC address table mapping addresses to physical ports. The destination MAC changes at each hop (set to the next router's MAC address), while source and destination IP addresses remain constant throughout the journey. firewall

At the Physical layer, Network Interface Cards (NICs) convert frames into electrical signals on copper (voltage levels representing 1s and 0s), light pulses in fiber optics (presence or absence of light), or radio frequency modulation for wireless (phase and amplitude shifts encoding bits). ITU Online The preamble allows receiver clock synchronization, while the Start Frame Delimiter signals the actual frame beginning. Sierrahardwaredesign

### De-encapsulation Reveals Data at the Destination

The receiving system reverses the process, with each layer removing its corresponding header. ComputerNetworkingNotes **Physical layer components detect the preamble pattern (alternating 1s and 0s), synchronize their clock, and recognize the Start Frame Delimiter** (ending in two consecutive 1s). They assemble the bit stream into a complete frame and pass it to the Data Link layer. firewall

The Data Link layer first verifies the destination MAC address matches its own interface (or is a broadcast address). If not, the frame is discarded immediately—no point wasting CPU cycles on traffic intended for other hosts. GeeksforGeeks **For matching frames, the layer recalculates the CRC-32 over the frame contents and compares it to the received Frame Check Sequence**. Mismatches indicate corruption during transmission, causing frame discard. The error is not corrected at this layer; upper layers (TCP) must detect missing data and request retransmission. firewall
...
The Transport layer adds end-to-end reliability where needed. **TCP's sequence numbers, acknowledgments, and retransmission mechanisms guarantee that data arrives complete and in order**, recovering from the Network layer's unreliable service. Flow control (window size) prevents fast senders from overwhelming slow receivers. Port numbers enable multiplexing—multiple applications on one host simultaneously using the network. UDP provides minimal overhead for applications that prefer speed over reliability. Neither lower nor upper layers can provide these services. Tech Invite ietf

This separation of concerns enables flexibility and modularity. Applications don't need to know whether they're running over Ethernet, Wi-Fi, or fiber optics—the lower layers abstract these details. Network hardware can be upgraded without changing application code. New protocols can be introduced at any layer independently. ITU Online **An HTTP server works identically whether traffic arrives over IPv4, IPv6, or even newer network protocols**, because the layering provides consistent interfaces between layers.
...
Display filters use PDU-specific field names. **Filtering "tcp.analysis.retransmission" shows only retransmitted TCP segments, indicating packet loss or timeout issues**. Filtering "icmp.type=3 and icmp.code=4" reveals "Fragmentation Needed and DF Set" messages that signal MTU problems. Microsoft Community Hub Understanding PDU structures enables precise filtering to isolate specific issues from massive capture files containing millions of packets.
...
IPsec provides network-layer encryption, encapsulating entire IP packets inside new IP packets with encryption applied. This protects against Layer 3 and below eavesdropping—an attacker capturing frames off the wire sees only encrypted payloads, not the original IP headers or TCP data. Wikipedia But IPsec cannot protect against attacks originating inside the encrypted tunnel endpoint. TLS operates at higher layers, encrypting application data before it reaches TCP, protecting against compromised infrastructure between client and server.
