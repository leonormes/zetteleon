...
According to this view, what we consciously perceive is not the raw sensory data, but our brain's best guess—its current working model. The only information that typically bubbles up to our conscious awareness is the "error signal," which is why we notice the unexpected far more than the expected.

This mechanism is what [[Prediction Error Breaks Maladaptive Feedback Loops|breaks maladaptive feedback loops]] and is the engine of learning. The biochemical consequence of this process is that [[Prediction Error Drives Dopamine Release]].
