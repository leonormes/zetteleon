...
> **Thesis:** Structure is not a representation of truth — structure *is* truth. This MoC is the hub for all systems — software, data, infrastructure, and knowledge — that derive their correctness from the mathematical properties of their shape, not from runtime validation or narrative description.
...
C12 relaxes the ty-th requirement. Formal methods here may be **relational** (SQL, CDM schema, formal data modelling) rather than type-theoretic. The key test: is the schema *normative* (defines valid states) rather than *descriptive* (annotates existing ones)?
...
C11 relaxes M8. Notes here govern systems that are *declarative and structurally grounded* but do not require formal proof machinery. The key test: is the state file / specification the *canonical* truth, such that divergence from it is definitionally a failure (not just unexpected behaviour)?
...
| Make Illegal States Unrepresentable | A HEAD note cannot be cited as a stable source. The ProdOS schema *physically encodes* epistemic validity. |
