---
tags: [domain/software-engineering, type/SoT, concept/complexity]
status: evergreen
trust-level: stable
synthesis-count: 2
...
*   Structural Complexity (Privacy & Governance): Handled by the *Data Architecture*.
    *   Nodes hold identifiable data (Invariant: Raw data never leaves).
    *   Hub receives only aggregates (Invariant: No PII in wire format).
*   Algorithmic Complexity (Statistics): Handled by the *Hub*.
