---
aliases: ["Read-Only OMOP", "Transient CTE Pattern", "Ephemeral Cohort Generation"]
confidence: "High"
created: 2026-01-06
epistemic: "Pattern"
last_reviewed: 
...
Since we cannot persist the *Result* (The Cohort Table) in the DB, we must persist the *Definition* (The Logic) in the App and stream the result on demand.
...
1.  **Vocabulary CTE:** Inject Concept Sets as a `VALUES` or `UNION ALL` block.
    *   *Standard:* `INSERT INTO #Codesets...`
    *   *Read-Only:* `WITH Codesets AS (SELECT 1 as id, 1234 as concept_id...)`
...
### B. SQL Optimization
*   **Predicate Pushdown:** Ensure date filters are applied in the *innermost* CTEs.
