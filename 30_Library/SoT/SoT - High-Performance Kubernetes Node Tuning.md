...
1. Thread Affinity: Bind worker threads to specific logical cores (Hyper-threads).
    - _Pattern:_ Place cooperative threads (Worker + Helper) on sibling hyper-threads (L1/L2 sharing).
    - _Pattern:_ Place contentious threads (Worker + Worker) on separate physical cores.
