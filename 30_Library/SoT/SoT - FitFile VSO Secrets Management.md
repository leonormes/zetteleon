...
modified: 2026-04-02T11:45:00+00:00
...
- **Selective Extraction (Transformation)**: Use VSO's `transformation.templates` to pull only required fields from multi-value Vault secrets.
    - *Example:* Pulling `gitlab_token` while leaving `admin_password` and `server_secret_key` securely in Vault.
...
*Consistency check*: Ensure `spec.namespace` in VSS/VDS matches the Vault-side path, not the K8s namespace.
