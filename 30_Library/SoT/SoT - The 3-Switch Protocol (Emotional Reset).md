---
aliases: [3-Switch Protocol, The 15-Second Reset, The Physiological Reset]
conformant: false
created: 2025-12-25T11:45:00+00:00
modified: 2026-09-14T11:52:52+00:00
non_conformance_reason: "Bulk inferred type. Needs review."
permalink: llmeon/30-library/so-t/so-t-the-3-switch-protocol-emotional-reset
tags: [emotional_regulation, protocol, TheHuman/Health/ADHD, TheHuman/Neuroscience]
title: SoT - The 3-Switch Protocol (Emotional Reset)
type: sot
---

## SoT - The 3-Switch Protocol (Emotional Reset)

> [!abstract] The Principle
> Action Precedes Emotion. The brain is a prediction machine that takes cues from the body. By manually overriding the physical signal, you force the chemical state to follow.
> Timeframe: ~15 Seconds.

---

### 1. The Protocol

#### Switch 1: The Face Feedback (Body-to-Brain)

_The Signal of Safety._

- Action:
    1. Raise eyebrows gently.
    2. Relax the jaw.
    3. Lift corners of mouth into a soft smile (Duchenne smile).
- Duration: Hold for 15 seconds.
- Mechanism: The facial feedback hypothesis. This specific muscular pattern signals "safety" to the nervous system, downregulating cortisol and upregulating serotonin.

#### Switch 2: The "Name It to Tame It" (Amygdala Dampening)

_The Logic Override._

- Action: Explicitly label the emotion. "I am feeling overwhelmed." "I am feeling defensive."
- Mechanism: Affect Labeling. As detailed in [[SoT - Journaling as Cognitive Engineering]], labeling an emotion shifts activity from the Amygdala (Reaction) to the Ventrolateral Prefrontal Cortex (Logic), physically reducing the intensity of the "panic loop."

#### Switch 3: The Micro-Win (Dopamine Injection)

_The Progress Signal._

- Action: Execute a task that takes <15 seconds.
    - _Examples:_ Take one deep breath. Drink a sip of water. Fix posture.
- Mechanism: The brain releases dopamine upon _any_ completion of a goal, regardless of size. This "Micro-Win" breaks the inertia of failure and creates a tiny spark of momentum.

---

### 2. Integration with ProdOS

This protocol serves as the "Emergency Brake" within the Ignition Protocol ([[SoT - PRODOS (System Architecture)]]).

- Trigger: When the "Wall of Awful" (ADHD Paralysis) prevents starting a task.
- Sequence:
    1. _Feel Stuck._
    2. Execute 3-Switch Protocol. (Reset State).
    3. Execute Ignition Hack. (Mystery/Time Trial/Spite).
    4. _Begin Work._

---

### 3. Related Concepts

- [[SoT - Journaling as Cognitive Engineering]] (Affect Labeling)
- [[SoT - The Cognitive Physiology of Task Execution]]
- [[Emotional Regulation Techniques for ADHD Productivity]]
