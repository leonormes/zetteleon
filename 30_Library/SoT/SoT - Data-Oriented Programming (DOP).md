...
see_also: ["[[SoT - Data-Centric Software Engineering]]", "[[SoT - Slot Map (Generational Arena)]]", "[[SoT - Rust Language]]", "[[SoT - Database Internals for Developers]]"]
source_of_truth: []
status: "stable"
...
### 5.1 The "Dirty Flag" List

Instead of checking every object `if (obj.isDirty)`, maintain a separate list of `DirtyIDs`.

* **Process:** Iterate only the `DirtyIDs` list.
* **Benefit:** Processing cost scales with *changes*, not *total entities*.
