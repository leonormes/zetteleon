...
| ID | Name | Binary Test |
|----|------|-------------|
| **M1** | Deterministic / Declarative | *Does this note describe a system whose outputs are fully determined by its specification, with no runtime variance?* |
| **M2** | Probabilistic / Stochastic | *Does this note reason about systems with inherent statistical or biological variance?* |
| **M4** | Structural Truth | *Is structure / schema the **primary** carrier of truth — not narrative, documentation, or convention?* |
| **M8** | Formal Methods | *Does this note employ at least one formal proof technique, type-theoretic construction, or formal relational schema as its **method** (not merely its subject)?* |
| **M10** | Complexity Reduction via Structure | *Does this note identify a class of complexity that the formal structure **eliminates** — not merely organises?* |
| **ty-th** | Type Theory | *Is type theory invoked as an **explicit framework** — Curry-Howard, dependent types, linear logic, HoTT — not as a metaphor?* |

**Derived discriminators (C16 only):**

| ID | Name | Binary Test |
|----|------|-------------|
| **M-sub** | Computational Substrate | *Is the formal system's realization dependent on a computational or physical substrate?* |
| **M-axiom** | Axiomatic Independence | *Does the formal system derive from first-principles axioms without reference to any substrate?* |
...
- [ ] **A2.** Ask: *Does this note reason about structure, formal systems, types, schemas, or declarative specifications?*
...
| **DOCUMENTATION_FALLACY** | Schema described as annotation of truth held elsewhere | Rewrite schema as normative: it must *define* valid states, not *describe* existing ones. If impossible, demote to C4. |
...
| **STRUCTURAL_NOMINALISM** | Note talks *about* structure without instantiating a structural claim | Rewrite to make a specific structural invariant, not a general claim about structure's value. |
| **STOCHASTIC_OVERRIDE** | M2 content at C4 drives conclusions | Add `fca/m2-framed` tag. Ensure M4 structural claim is the note's *conclusion*, not a framing device. |
...
- **DS = YES.** Rust ownership rules are enforced by the *compiler*. Data-Oriented Design is grounded in *cache line physics*. The formal system cannot exist without a substrate.
...
| *(unset)* | Sits at C16 join — bridges both | [[MOC - Type Theory]], [[SoT - Conservation of Complexity]] |
