...
**The distinction:**
- **Prompt Engineering**: Trying to *talk* the model into following a methodology.
- **Flow Engineering**: Building a deterministic wrapper that *forces* the model to comply, making non-compliance structurally impossible.
...
1. Orchestrator sends prompt: *"Here is a requirement. Output exactly one failing unit test. Return ONLY valid code. No markdown, no implementation."*
2. Orchestrator writes output to `test_feature.py`.
3. **Mechanical Gate**: Run `pytest test_feature.py`.
   - Test **passes** → LLM hallucinated implementation or wrote a vacuous test. Feed result back: *"Error: test must fail. Rewrite."* Loop.
...
2. Feed *only* the failing test code and error trace: *"Write the minimum implementation to make this test pass. Output ONLY implementation code."*
...
1. Feed passing implementation: *"Optimise this code for readability. Do not change the function signature."*
