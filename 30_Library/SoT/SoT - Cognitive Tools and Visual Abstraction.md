...
1. Scientific Discovery: The process of identifying useful abstractions to explain the world (making the invisible visible).
2. Cognitive Tools: The artifacts created to capture these abstractions (e.g., the number line, the periodic table).
3. Engineering Application: Leveraging these tools to build new technologies, which in turn enable further discovery.
...
- _Principle:_ Increasing abstraction often increases communicative precision.
- _Example:_ A schematic diagram of a subway system is more useful for navigation than a satellite photograph of the tunnels.
...
- Audience Sensitivity: Humans intuitively select plot types (bar vs. scatter) based on the _epistemic goal_ of the viewer (e.g., "comparison" vs. "trend").
- The AI Gap: Current AI models (Vision-Language Models) struggle with "visual reasoning." They often fail to replicate human error patterns or understand the _intent_ behind a graph's design, focusing instead on pixel-level patterns.
...
- Edwin Hutchins: Distributed Cognition ("Cognitive Ethnography").
