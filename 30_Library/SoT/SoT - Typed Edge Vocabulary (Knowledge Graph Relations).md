...
modified: 2026-09-14T11:52:55+00:00
...
2. A content-block—a bare id. The `id` of a `<!--content-block-start … id="…"-->` block anywhere in the vault. Blocks are not files, so they cannot be linked; this is the only case where a bare target is valid, and the compiler warns if a _note_ target is written bare.
...
The compiler (§6) fails an edge whose target resolves to nothing (a _dangling edge_) and warns on one that resolves ambiguously (same id in two places).
...
The prose and frontmatter forms are not deprecated: §2 already prefers the frontmatter field for note→note relations, and a prose tension records _why_ two notes conflict, which no edge attribute captures. They are complementary, not rivals. The visible `rel::` form in MoCs is the only genuine near-duplicate of §1, and migrating it is optional—a decision to make only if those MoCs start carrying argument weight.
