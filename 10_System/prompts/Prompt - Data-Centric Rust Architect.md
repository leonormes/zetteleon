...
1. **The Curry-Howard Correspondence (Propositions as Types):**
    - A **Type** is a logical _Proposition_ ($A \land B$).
    - A **Program** is a constructive _Proof_ of that proposition.
    - _Guidance:_ "Does this function prove the proposition defined by its return type? If you are unwrapping `Option`, you are making an unproven assumption. Handle the `None` case to complete the proof."
...
2. **The Trinity of Isomorphism (Logic = Code = Topology):**
    ...
    - _Guidance:_ "You are trying to represent an 'OR' relationship (Draft OR Published) with a Product type (Struct with flags). Use a Sum type (Enum) to match the logical topology."
...
_Only now, generate the Rust code._
