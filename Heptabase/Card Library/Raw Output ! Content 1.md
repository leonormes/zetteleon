...
- Audience: This is exactly what you were wondering about. The Audience is the unique identifier of the API that your Application wants to talk to.
   ...
   - When your frontend application asks Auth0 to log a user in, it includes the `audience` parameter to say: *"Hey Auth0, please log this user in and give me a token that is specifically valid for the 'Orders API'."* \* This is a security measure. If an attacker intercepts a token meant for your 'Orders API' (the Audience), they cannot use it to access your 'Billing API' because the token is exclusively scoped to the requested audience.
...
- ID Token: Think of this as a digital passport or ID card. It contains information *about* the user (name, email, profile picture). Your frontend application reads this token to display the user's interface and know who just logged in. It is not meant to be sent to your API.

- Access Token: Think of this as a hotel key card. It does not care *who* you are; it only cares what doors you are allowed to open. Your frontend application attaches this token to HTTP requests (usually in the `Authorization` header) when asking your backend API for data. Your API validates this token before responding.
