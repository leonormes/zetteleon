...
with the Quickfix List). We can browse the results by running :copen, which opens the quickfix window. But instead of stepping through the results one at a time, we want to run the substitute command on every file that appears in the [quickfix list.md](quickfix%20list.md). We can do so using the :cfdo command (see :cfdo[ⓘ](%3Chttp://vimhelp.appspot.com/quickfix.txt.html#%3Acfdo%3E)).
