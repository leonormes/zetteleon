Part of [The Person_ID Handbook](https://digital.nhs.uk/services/personal-demographics-service/master-person-service/the-person_id-handbook)
...
![](https://digital.nhs.uk/binaries/content/gallery/website/services/personal-demographics-service/master-person-service-mps/february/figure-1.svg)

Creation of Person_ID via MPS process flow (high-level)
...
The MPS record bucket in the Spine’s data stfore is used to link records for the same person that cannot be traced in PDS. If a match is found, an MPS_ID is returned, otherwise, the algorithm considers whether to create a new MPS_ID. MPS_ID is also known as UPRI 1 in certain documentations.
...
![](https://digital.nhs.uk/binaries/content/gallery/website/services/personal-demographics-service/master-person-service-mps/february/figure-2.svg)

Cross-check trace process flow in DPS core

**Figure 2. Cross-check trace process flow in DPS core**

![](https://digital.nhs.uk/binaries/content/gallery/website/services/personal-demographics-service/master-person-service-mps/february/figure-3.svg)

Cross-check trace process flow in Spine
...
- D <sup>1</sup> D <sup>2</sup> being swapped to D <sup>2</sup> D <sup>1</sup>, for example, 12 becomes 21

- Y <sup>1</sup> Y <sup>2</sup> Y <sup>3</sup> Y <sup>4</sup> being swapped to Y <sup>1</sup> Y <sup>2</sup> Y <sup>4</sup> Y <sup>3</sup>, for example, 1945 becomes 1954
...
- Royal Mail’s Postcode Address File <sup>2</sup> (PAF) address

Footnote 2

2\. [Postcode Address File <sup>2</sup> (PAF) address](https://www.poweredbypaf.com/)
...
![](https://digital.nhs.uk/binaries/content/gallery/website/services/personal-demographics-service/master-person-service-mps/february/figure-4.svg)

Alphanumeric trace process flow in Spine
...
![](https://digital.nhs.uk/binaries/content/gallery/website/services/personal-demographics-service/master-person-service-mps/february/figure-5.svg)

Algorithmic trace for all records including HES
...
Before the blocking stage takes place, family name and given names are pre-processed by removing spaces and hyphens and then mapping via a dictionary into a normalised name <sup>3</sup>. The algorithmic traces consider the following blocks:
...
3\. The dictionary maps input names like “Jenny” to the full form “Jennifer”. The full content of the dictionary is available in the [supplementary material](https://digital.nhs.uk/binaries/content/assets/website-assets/services/mps/name_mapping.csv).
...
![](https://digital.nhs.uk/binaries/content/gallery/website/services/personal-demographics-service/master-person-service-mps/february/figure-6.svg)

Compute postcode score in algorithmic trace process flow
...
![](https://digital.nhs.uk/binaries/content/gallery/website/services/personal-demographics-service/master-person-service-mps/february/figure-7.svg)

Compute name score in algorithmic trace process flow
...
![](https://digital.nhs.uk/binaries/content/gallery/website/services/personal-demographics-service/master-person-service-mps/february/figure-8.svg)

MPS_ID matching process flow
...
The results from MPS are provided in the response file. The response file is only used in DPS for internal purposes (for example, creating the final Person_ID field, as explained in the [Person_ID creation and data set enrichment section.](https://digital.nhs.uk//services/personal-demographics-service/master-person-service/the-person_id-handbook/technical-details-of-the-data-linkage-algorithm#person_id-creation-and-data-set-enrichment)
...
|---|---|---|---|---|---|---|---|
| MATCHED\_  NHS_NO | NHS num | NHS num | NHS num | NHS num | 0000000000 | 9999999999 | 0000000000 | 
| MPS_ID | Empty | Empty | Empty | Empty | Present | Empty | Empty | 
| Matched  Algorithm  Indicator | 1 | 1 | 3 | 4 | 1,3,4 | 4 | 0,1,3,4 | 
| Matched  Confidence  Percentage | 100 | 100 | 100 | 50\*-100 | 0 | 0 | 0 | 
| FamilyNameScore  Percentage | null | 0 | 0 | 0-100 | 0 | 0 | 0 | 
| GivenName  Score  Percentage | null | 0 | 0 | 0-100 | 0 | 0 | 0 | 
| DateOfBirth  Score  Percentage | null | 0 | 0 | 0-100 | 0 | 0 | 0 | 
| Gender  Score  Percentage | null | 0 | 0 | 0-100 | 0 | 0 | 0 | 
| Postcode  Score  Percentage | null | 0 | 0 | 0-100 | 0 | 0 | 0 | 
...
| Row number | Person_ID | Matched_NHS_No | MPS_ID (UPRI 1) | One-time-use ID (UPRI 2) | 
|---|---|---|---|---|
...
- 
   ...
   - **Row number** 2
      ...
      - **Person_ID** A987654321

      - **Matched_NHS_No** 0000000000

      - **MPS_ID (UPRI 1)** A987654321
...
- 
   ...
   - **Row number** 3
      ...
      - **Person_ID** A123454321

      - **Matched_NHS_No** 0000000000

      - **MPS_ID (UPRI 1)** A123454321\~\~\~ A987656789
...
- 
   ...
   - **Row number** 4
      ...
      - **Person_ID** U123123123

      - **Matched_NHS_No** 9999999999
...
- 
   ...
   - **NHS number** 0123456789
      ...
      - **Token_Person_ID** 987A543219876G432198RR5432198765
...
- 
   ...
   - **NHS number** -
      ...
      - **Token_Person_ID** 647A5142198RRT432198RHH432112332

- 

   - **NHS number** -

      - **Person_ID** U123456789

      - **Token_Person_ID** 1017JJ146HH8R12322198RHYY771KK32
...
![](https://fra1.qualtrics.com/WRQualtricsSiteIntercept/Graphic.php?IM=IM_eCGDlRAkR5KXvSe)
