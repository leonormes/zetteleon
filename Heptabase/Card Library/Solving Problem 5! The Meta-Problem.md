...
Here is what the whole thing looks like in practice:
...
└────────┬────────┘
         ...
         ▼
┌─────────────────────────┐
│  CAPTURE: open inbox,   │   ← 10 seconds, no decisions
│  type, close. Done.     │
└────────┬────────────────┘
         │
         ▼
    (carry on with
     what you were
      doing)
         ...
         ▼
┌─────────────────────────┐
│  PROCESS: triage inbox  │   ← 15 minutes, dedicated session
│  - task? → do/list      │
│  - delete? → delete     │
│  - seed? → make card    │
│  - unclear? → wait once │
└────────┬────────────────┘
         ...
         ▼
┌─────────────────────────┐
│ CLAIM/CONCEPT/QUESTION  │   ← sharp, falsifiable, dated,
│ card created, done      │     linked. Done for now.
│ (by the 5-box test)     │
└─────────────────────────┘

```

Three stages. Two protocols. One stopping rule per stage. That is the whole workflow.