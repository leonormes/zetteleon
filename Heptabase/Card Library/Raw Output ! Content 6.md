...
- Terrence Deacon, \_The Symbolic Species\_—argues human brains and human languages co-evolved, each shaping the other. Exactly your intuition, made rigorous.
...
2. However, the fact that children do it with *millions* of times less data suggests that while the LLM proves it's *possible*, it also proves that humans are doing it in a fundamentally different, more efficient way.
...
Deacon, however, pushes back on the idea of a completely pre-wired "language organ." He posits that the extra support for language learning is vested neither entirely inside the child's brain nor in the parents, but outside the brain, *in the language itself*. He suggests that universal rules of grammar have emerged spontaneously and independently in each evolving language in response to universal biases in human learning and memory.
...
You are entirely right to point out that grouping Pinker and Deacon together under the umbrella of "evolutionary linguistics" masks a profound, almost diametrically opposed disagreement about *where the heavy lifting actually happens*.
...
Pinker's View (The Biological Mandate): Pinker, extending Chomsky, argues that the structure of language is biological. In this view, the human brain contains a genetically specified "language organ" pre-loaded with Universal Grammar. A child learns language easily because the cognitive wiring is already fully formed, waiting only for local vocabulary to trigger the correct switches. The structure is in the *brain*.

Deacon's View (The Cultural Symbiote): Deacon argues that the structure of language is cultural and environmental. The brain didn't evolve a specific grammar module; rather, the brain evolved general symbolic capacity (driven by the prefrontal cortex), and *languages themselves evolved* to fit that specific cognitive landscape. Language is like a highly adapted symbiote or parasite that has evolved to be perfectly digestible by the immature human mind. The structure is in the *language*.
