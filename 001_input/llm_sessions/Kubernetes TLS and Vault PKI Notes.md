...
review_interval: 
see_also: []
source: "https://gemini.google.com/share/d6e9f3d9269b"
source_of_truth: []
status: 
tags: ["clipped", "llm_session"]
title: Kubernetes TLS and Vault PKI Notes
type:
uid: 
updated: 
version:
...
Create new zettelkasten atomic markdown notes from all this out put fire my obsidian pkm
...
## Vault AppRole (vault\_approle\_auth\_backend\_role)
...
## AppRole SecretID (vault\_approle\_auth\_backend\_role\_secret\_id)
...
2. **Create the CA Bundle**: The intermediates and root must be concatenated into a single file. The order does not strictly matter for `openssl verify`, but conventionally the root is last.
 Bash
...
1. Double-click the `rootCA.crt` file (you may need to rename it from `.pem`).
2. Click the **Install Certificate...** button.
3. Choose the **Local Machine** store location and click Next.
4. Select **Place all certificates in the following store**.
5. Click **Browse...** and select the **Trusted Root Certification Authorities** store.
6. Click Next, then Finish. Acknowledge the security warning.
...
curl: (35) LibreSSL/3.3.6: error:1404B410:SSL routines:ST\_CONNECT:sslv3 alert handshake failure
