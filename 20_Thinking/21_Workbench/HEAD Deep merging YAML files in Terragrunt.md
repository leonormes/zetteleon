...
status: "processing"
tags: ["input"]
title: HEAD Deep merging YAML files in Terragrunt
type: "head"
---
...
The Terragrunt `deep_merge()` runs at the HCL evaluation layer _before_ Terraform even sees the values, so you can use the merged result directly in `inputs` without adding a module dependency. It also means `terraform plan` sees a flat, already-resolved map—easier to debug drift.
