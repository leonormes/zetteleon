...
## MOC - Atomic Knowledge Cleaver System

The Atomic Knowledge Cleaver is a process and philosophy for maintaining a high-integrity personal knowledge base. It rests on a fundamental principle: [[Atomic vs Structural Notes]] rel:: part-of. This binary category invariant prevents knowledge bases from becoming either isolated facts with no coherence or narratives with no verifiable building blocks.

### The Core System

When you encounter a long note that mixes facts with reasoning, you have a hybrid note that needs cleaving. You follow [[Atomic Cleaving Process]] rel:: example-of, which uses a rigorous three-phase approach: analysis (identify facts and context), deduplication (check for existing atomic notes), and generation (build new atoms and a MOC).

This process works because it honors the category invariant. Every extracted [[Atomic vs Structural Notes]] rel:: part-of is a pure, context-free building block. Every structural note (the MOC) exists solely to create context by linking these atoms with [[Typed Links for Knowledge Context]] rel:: supports that add semantic meaning to each connection.
...
The binary category invariant [[Atomic vs Structural Notes]] rel:: defines means atomic notes never link to structural notes. This prevents "circular reasoning" in your knowledge base and keeps the architecture clean.
