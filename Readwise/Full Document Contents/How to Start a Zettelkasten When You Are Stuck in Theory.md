---
created: 2026-03-14T09:50:14+00:00
modified: 2026-03-14T11:09:22+00:00
tags: [articles]
title: How to Start a Zettelkasten When You Are Stuck in Theory
---

## How to Start a Zettelkasten When You Are Stuck in Theory

![rw-book-cover](https://zettelkasten.de/favicon.ico)

### Metadata

- Author: [[Dear Zettlers]]
- Full Title: How to Start a Zettelkasten When You Are Stuck in Theory
- Category: articles
- Summary: Start with a simple app and learn only how to make notes, link them, and search. Build one focused structure note for a real research interest and keep expanding it by adding and linking clear, revised notes. When that interest is explored, pick a new one and connect useful old notes to your current work.
- URL: <https://newsletter.christiantietze.de/w/C7fNf892atTMR3q87ZOlSNLQ/vGdm8DaHEoRuG7zMEPyC3w/wJi2wuruI03bcrdQTU2sQw>

### Full Document

This guide is deliberately concise. It is written for people who struggle to get started and get stuck endlessly consuming Zettelkasten theory.

1. Start with a minimalist, distraction-free app.
2. Learn _only_ how to quickly create new notes, set direct links, and find notes in the Zettelkasten (full-text search, following links).
3. Create a [structure note](https://newsletter.christiantietze.de/l/C7fNf892atTMR3q87ZOlSNLQ/Ye763kVDb892iM3WLB1xdoW7mQ/wJi2wuruI03bcrdQTU2sQw) for a current research interest.
4. Create notes from a link first: start by adding a link on the structure note to the future note that does not yet exist, and then create the corresponding note.

Your first and only goal initially is to make the structure note a representation of your research progress that you're proud of. Imagine someone asks you about the topic, and you feel confident pulling up the structure note while they see it. This structure note should give the impression, "Wow, there's a lot in there."

Focus on this one structure note. This indirectly means: focus on this one research interest.

This approach is inspired by Josh Waitzkin's explanation in _The Art of Learning_ about how to start learning chess. Instead of beginning with the full board and all pieces, you start with just the king and one other piece. This allows you to focus on the behavior of a single piece. You isolate a sub-skill and can train it more effectively.

When building this one structure note, ensure each individual note is carefully crafted. Use paragraphs, revise your wording until it's clear, put effort into the title (and adjust it later if it doesn't fit—it happens often!), and try to connect each idea with other ideas in your Zettelkasten. This network of connections is straightforward when you focus on one research interest: each link should lead to another thought that expands the current one. This could be a counterposition, an alternative definition of a term, a practical implication, a template based on the idea, etc.

Your research interest should be genuine. It's a mistake to choose something you think is "safe" to fail at. It may seem rational to experiment with something where failure has no serious consequences (e.g., having to start over). But this only leads to a lack of motivation to build the intensity and seriousness needed for thorough work. If the content of your Zettelkasten isn't important to you and only serves to learn the Zettelkasten method, you'll quickly become superficial and sloppy with the content. The Zettelkasten method requires serious engagement with the content—it cannot be learned with dummy content.

Once you've sufficiently pursued your research interest, choose a new one and repeat the process. You'll encounter notes from your previous research interest. Always ask yourself if those old notes can help advance your current research. Can you benefit from a concept you've already captured? Do you see a pattern between an old note and a new one? Record all of this in your Zettelkasten.

Get started.

Live long and prosper

 Sascha

#### Lately on the Blog

- [One (and another) Gear in the Zettelkasten Machine: A Deep Dive into a Key Mechanism](https://newsletter.christiantietze.de/l/C7fNf892atTMR3q87ZOlSNLQ/vVOn2LjLn5fBvHj6tiQJ4g/wJi2wuruI03bcrdQTU2sQw)
 Each element of your workflow has a specific effect. You might compare it to key training sessions like explosive strength training or circuit training. Together they make (or break) your practice.
- [Optimal Concentration: Dedicated Sessions Are Your Success Recipe](https://newsletter.christiantietze.de/l/C7fNf892atTMR3q87ZOlSNLQ/f763ZdS9Antkq4blx4k9Tdyw/wJi2wuruI03bcrdQTU2sQw)
 How to design a deep knowledge work session with your Zettelkasten. The goal is to maximize the time spend in deep focus.

[View web version](https://newsletter.christiantietze.de/w/C7fNf892atTMR3q87ZOlSNLQ/vGdm8DaHEoRuG7zMEPyC3w/wJi2wuruI03bcrdQTU2sQw)

You have received this email because you subscribed to our newsletter at zettelkasten.de. To stop receiving email [unsubscribe here](https://newsletter.christiantietze.de/unsubscribe/IvDO9763B2sIvkt5ZDx763oRy3SdrOaz7DZfGUvojEsaP892Q/vGdm8DaHEoRuG7zMEPyC3w/wJi2wuruI03bcrdQTU2sQw).

Our mailing address:

 Christian Tietze

 Dostalstr. 18

 D-33647 Bielefeld, Germany
