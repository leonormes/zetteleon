...
1. _CSI SnapshotMetadata Service API_: An API, offered by gRPC, that provides volume snapshot and changed block data.
2. _SnapshotMetadataService API_: A Kubernetes CustomResourceDefinition (CRD) that advertises CSI driver metadata service availability and connection details to cluster clients.
3. _External Snapshot Metadata Sidecar_: An intermediary component that connects CSI drivers to backup applications via a standardized gRPC interface.
