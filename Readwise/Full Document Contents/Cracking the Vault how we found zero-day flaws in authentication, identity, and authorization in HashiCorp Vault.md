...
tags: [articles]
title: Cracking the Vault how we found zero-day flaws in authentication, identity, and authorization in HashiCorp Vault
---
...
1. Code validation occurs using the `ValidateCustom()` function:
...
1. If the code is valid, it is marked as used:
...
1. `ldap` attack path (config-dependent)

This scenario depends on specific but common configuration choices

1. `cert` attack path (targeted impersonation)
