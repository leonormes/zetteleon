...
| **Con** | `labelSelectors` in AKS Backup filters *resources within* included namespaces, **not** namespace selection — misuse risks silent exclusion; requires kubectl label applied before backup runs | Config must be updated for each new namespace |
